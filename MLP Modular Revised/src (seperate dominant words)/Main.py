...
w2v_model = gensim.models.KeyedVectors.load_word2vec_format('GoogleNews-vectors-negative300.bin', binary=True, limit=500)
# w2v_model = gensim.models.KeyedVectors.load_word2vec_format('GoogleNews-vectors-negative300.bin', binary=True)
fake_w2v_model = w2v_model
# get data
df=pd.read_csv('data/data.csv',sep="\t", nrows=1000).drop(['PhraseId', 'SentenceId'], axis=1)
...
for x in range(0, data_divide_constant):
    ...
    mlp_feed_df = io_repo.save([mlp_train_input, mlp_train_output], turn_path+'/mlp_feed_df.csv')
    # normalize train set
    mlp_model, scaler = calculate_repo.getModelAndScaler(mlp_feed_df.iloc[:,:-1], mlp_feed_df.iloc[:,-1])
    ...
    calculated_test_scores_df = calculate_repo.calculate_test(test_df, train_df, train_word_df, w2v_model,
                                                              dominant_word_df, mlp_model, scaler)
