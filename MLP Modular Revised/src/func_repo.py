...
def clean_data (df):
    ...
    stopwords_set = set(line.strip() for line in open('english_redesigned'))
    # stopwords_set = set(stopwords.words('english_redesigned'))
    ps = PorterStemmer()
    df.Phrase = df.Phrase.str.replace('[^\w\s]', '')
    def stemm_stop_erase(x):
        proper_words = []
        words = x.split()
        for word in words:
            stemmed_word = ps.stem(word)
            if not stemmed_word in stopwords_set:
                proper_words.append(stemmed_word)
        return " ".join(proper_words)
    df.Phrase = df.Phrase.map(stemm_stop_erase)
    # Remove punctuations.
...
def fully_clean_df (df):
    ...
    df= df.groupby(['Phrase'])['Sentiment'].mean().reset_index()
    df = df[df['Phrase'].apply(lambda x: type(x) == str)]
    df = df.loc[df['Phrase'].str.len() > 0]
    # clean all NaN rows
    df = df.dropna()
    df = df.reset_index(drop=True)
    return df
...
def get_required_df_lists (clean_df_list, fully_clean_df_list, word_df_list, fake_data_divide_constant, index):
    ...
    return test_df_list, train_df_list, train_word_df_list
...
def get_required_fake_dfs (train_df_list, train_word_df_list, index):
    ...
    return fake_test_df, fake_train_df, fake_train_word_df

# take df lists in order given
# then return df versions by concatenating each ofthem
def dflists_to_dfs (test_df_list,train_df_list, train_word_df_list):
    ...
    return test_df, train_df, train_word_df
...
def old_normalize_inp_out_lists (mlp_train_input, mlp_train_output,divide_constant_list):
...
def normalize_inp_out_lists (mlp_train_input, mlp_train_output,divide_constant_list):
    if not divide_constant_list:
        ...
        # for input matrix
        for x in range (0,len(mlp_train_input[0])):
            feature = [item[x] for item in mlp_train_input]
            mean = np.mean(feature)
            std = np.std(feature)
            divide_constant_list.append([mean,std])
            for y in range (0, len(mlp_train_input)):
                mlp_train_input[y][x] = (mlp_train_input[y][x]-mean)/std+0.5
        # for output matrix
        mean = np.mean(mlp_train_output)
        std = np.std(mlp_train_output)
        divide_constant_list.append([mean,std])
        for y in range(0, len(mlp_train_output)):
            mlp_train_output[y] = (mlp_train_output[y]-mean)/std+0.5
        return mlp_train_input, mlp_train_output, divide_constant_list
    else:
        for x in range (0,len(mlp_train_input[0])):
            for y in range (0, len(mlp_train_input)):
                mlp_train_input[y][x] = (mlp_train_input[y][x]-0.5)*divide_constant_list[x][1]+divide_constant_list[0]
        # for output matrix
        for y in range(0, len(mlp_train_output)):
            mlp_train_output[y] = (mlp_train_output[y]-0.5)*divide_constant_list[-1][1]+divide_constant_list[-1][0]
        return mlp_train_input, mlp_train_output
